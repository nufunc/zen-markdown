...
이 약속은 호스트가 강제한다. 웹뷰가 보낸 진단 메시지는 `src/hostLogic.ts`의 `sanitizeDiag`를 거친 뒤에 기록된다.
`ev`는 영문 소문자와 밑줄로 된 64자 이하만 받는다. 필드는 유한한 숫자와 불리언, 허용 목록에 있는 문자열 값(`choice`의
`external`과 `mine`), `DriftKind` 이름을 키로 한 `kinds`만 남는다. 그 밖의 필드는 버리고 개수만 `dropped`로 남긴다.
`doc`과 `ts`는 웹뷰가 보내도 버리고 호스트의 해시와 기록 시각을 쓴다. 새 문자열 필드를 기록하려면
`sanitizeDiag`의 허용 목록에 값을 더해야 한다.
