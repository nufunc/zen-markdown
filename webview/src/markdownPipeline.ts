...
export function blocksToMarkdown(blocks: any[], toMarkdown: (blocks: any[]) => string, quoteJoins?: Set<string>, tables?: Map<string, TableOriginal>): string {
  return compactTables(restoreTableBreaks(restoreQuoteStructures(dividersAsDashes(serializeKeepingListChildren(processBlocksToMarkdown(blocks, quoteJoins), toMarkdown)))), blocks, tables);
}
